...
import '../../features/accounts/presentation/cubit/accounts_overview_state.dart';
  ...
  AppRouter({required AuthCubit authCubit, required UsersRepository usersRepository})
      : router = GoRouter(
          ...
          routes: [
            ...
            GoRoute(
              path: AccountsOverviewPage.path,
              name: AccountsOverviewPage.name,
              builder: (_, state) {
                final filter = state.uri.queryParameters['filter'];
                final initialFilter = filter == 'with_debt'
                    ? AccountsOverviewFilter.withDebt
                    : (filter == 'up_to_date' ? AccountsOverviewFilter.upToDate : AccountsOverviewFilter.all);
                return AccountsOverviewPage(initialFilter: initialFilter);
              },
            ),
            ...
            GoRoute(
              path: PedidosPage.path,
              name: PedidosPage.name,
              builder: (_, state) => PedidosPage(
            ...
            GoRoute(
              path: StockPage.path,
              name: StockPage.name,
              builder: (_, state) => StockPage(initialFilter: state.uri.queryParameters['filter']),
            ),
