  ...
  async getTopProducts(query, user) {
    ...
    return {
      ...
      topByRevenue: [...topByQuantity.rows]
        ...
        .map((r) => ({ productId: r.product_id, productName: r.product_name, revenue: toNum(r.revenue) })),
      topByProfit: [...topByQuantity.rows]
        .sort((a, b) => toNum(b.profit) - toNum(a.profit))
        .map((r) => ({ productId: r.product_id, productName: r.product_name, profit: toNum(r.profit) })),
